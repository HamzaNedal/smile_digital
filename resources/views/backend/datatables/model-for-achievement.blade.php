      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
            <div class="form-group">
                <label for="recipient-name" class="col-form-label">هذا القسم يحتوي على إنجازات </label>
                <small> هل تريد نقل الإنجازات لقسم أخر؟</small>
              </div>

            <div class="input-group">
                <input type="hidden"  id="old_category_id" value="{{ $data->id }}">
                <select class="custom-select" id="new_category_id" name="category_id" aria-label="Example select with button addon">
                    @foreach ($categories as $category)
                        <option value="{{ $category->id }}">{{ $category->translation->where('lang_code','ar')->first()->name }}</option>
                    @endforeach
